...
Date Date::fromIso(std::string_view iso) {
    if (iso.size() != 10 || iso[4] != '-' || iso[7] != '-') return Date{};
    int y = 0, m = 0, d = 0;
    ...
    for (size_t i = 0; i < 4; ++i) if (!digit(iso[i],     y)) return Date{};
    for (size_t i = 5; i < 7; ++i) if (!digit(iso[i],     m)) return Date{};
    for (size_t i = 8; i < 10; ++i) if (!digit(iso[i],    d)) return Date{};
    Date out{y, m, d};
...
std::string Date::toHuman() const {
    ...
    if (month < 1 || month > 12) return toIso();
    char buf[24];
...
bool operator<=(Date a, Date b) { return a < b || a == b; }
